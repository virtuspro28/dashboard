...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (remotesData.success) setRemotes(remotesData.data);
      } catch (error) {
        if (!disposed) {
  ...
  const healthyDisks = stats.storage.healthyDisks || hardware.disks.filter((disk) => disk.status === 'PASSED' || disk.status === 'OK').length;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-slate-900/40 backdrop-blur-md border border-white/5 rounded-[2.5rem] p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {containers.filter((container) => container.state === 'running').slice(0, 4).map((container) => (
