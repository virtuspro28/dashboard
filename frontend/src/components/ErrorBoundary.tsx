...
export default class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.error("ErrorBoundary atrapó un error de UI:", error, errorInfo);
  }
      ...
      return (
        <div className="flex min-h-screen items-center justify-center bg-slate-950 px-6">
          <div className="w-full max-w-xl rounded-[2rem] border border-red-500/20 bg-slate-900/70 p-8 text-center backdrop-blur-md">
            ...
            <h1 className="mt-3 text-2xl font-black text-white">La interfaz encontró un error</h1>
            <p className="mt-3 text-sm leading-relaxed text-slate-400">
              Se ha evitado la pantalla en blanco. Recarga la página o revisa el componente que falló.
            </p>
