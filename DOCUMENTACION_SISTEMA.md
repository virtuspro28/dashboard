# Documentación del Sistema HomePiNAS

Este documento resume la arquitectura de despliegue, persistencia de datos y gestión de permisos del entorno HomePiNAS. 

## 1. App Store: Cómo añadir una nueva app oficial manualmente

La App Store de HomePiNAS ha sido reescrita para depender exclusivamente de fuentes oficiales y originales (Docker Hub, GitHub Container Registry) abandonando la dependencia en el catálogo de terceros (CasaOS). 
...
```
3. Guarda el archivo, vuelve a compilar el backend (`npm run build`) y reinicia el servicio (`sudo systemctl restart homepinas.service`). La app aparecerá instantáneamente en la interfaz lista para desplegarse mediante su propio archivo `docker-compose.yml` que HomePiNAS generará en tiempo real.
...
* **Ruta Maestra Configurada:** Definida en el entorno, por defecto mapeada típicamente a `/opt/homepinas/data` (o `/home/sr_android/homepinas/data` si has modificado el directorio en la UI/variables de entorno).
* **Manifiestos generados:** Cuando instalas una app, HomePiNAS genera de forma invisible un archivo compose estándar en la subcarpeta `/store-manifests/<app_id>/docker-compose.yml`.
...
Simplemente copia la carpeta maestra de datos. Contiene tanto la base de datos de HomePiNAS como las configuraciones internas de todos tus contenedores.
...
Si arrancas HomePiNAS manualmente usando `npm run dev` o `npm start` bajo tu cuenta de usuario (ej. `sr_android` o `pi`), tu cuenta no tiene permisos administrativos nativos sobre el motor de Docker.
...
4. Vuelve a arrancar HomePiNAS. Ahora la aplicación podrá interactuar libremente con Docker sin chocar con bloqueos del kernel.
